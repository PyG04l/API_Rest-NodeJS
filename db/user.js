...
//Devuelve nombre del avatar de un usuario
const imageExistence = async (id) => {
  let connection;

  try {
    connection = await getConnection();

    const [imageName] = await connection.query(
      `
...
//Modifica el usuario
const modUser = async (id, al, em, pssO, pssN, bio, newAvatar, fc) => {
  let connection;
  ...
  try {
    ...
    if (pssO && pssN) {
      //falta comparar con la contraseña vieja
  ...
  try {
    ...
    if (theArgs.length > 1) {
      for (let i = 0; i < theArgs.length; i++) {
        if ((theArgs[i] != null || theArgs[i] != "") && i != 0) {
...
*/
//devuelve la id de un usuario buscado por email
const getCreatorById = async (idU) => {
  let connection;
  try {
    connection = await getConnection();
    ...
    const [creator] = await connection.query(
      `
        SELECT alias, email FROM users_info WHERE id_user = ?
        `,
      [idU]
    );

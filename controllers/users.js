...
const { createUser, checkUser, getUserById, checkAndChangePss, modUser } = require('../db/user');
...
const loginController = async (req, res, next) => {
    try {
        ...
        if (compEmail && compPass) {
            ...
            const token = jwt.sign({id: id}, process.env.SECRET, {expiresIn: '15d'});
...
const modifyUserController = async (req, res, next) => {
    try {
        const { id = 0, al = null, em = null, pss = null, bio = null, fp = null, fc = null} = req.body;
